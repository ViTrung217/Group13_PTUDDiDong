...
public class HistoryFragment extends Fragment {
    ...
    public void onResume() {
        super.onResume();
        ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle("Danh sách món ăn đã xem");
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        // Khởi tạo giao diện từ layout fragment_history
    ...
    private void loadViewHistory() {
        ...
        if (userId != -1) {
            ...
            Cursor cursor = dbHelper.getViewHistoryByUserId(userId);
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    int foodId = cursor.getInt(cursor.getColumnIndexOrThrow(DatabaseHelper.COL_VIEW_FOOD_ID));
                    String timestamp = cursor.getString(cursor.getColumnIndexOrThrow(DatabaseHelper.COL_VIEW_TIMESTAMP));
                    Food food = getFoodDetails(foodId);
                    if (food != null) {
                        food.setTimestamp(timestamp); // Gán timestamp từ view_history
            ...
            guestFoodAdapter = new GuestFoodAdapter(getContext(), historyList);
